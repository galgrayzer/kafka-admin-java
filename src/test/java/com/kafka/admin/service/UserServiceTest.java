...
class UserServiceTest {
    ...
    void testCreateUserWithScramSha512() throws Exception {
        ...
        // Then
        verify(admin).alterUserScramCredentials(argThat(operations -> {
    ...
    void testDeleteUserSuccess() throws Exception {
        ...
        // When
        userService.deleteUser("test-user", "localhost:9092", null, null, null, null);
    ...
    @Test
    void testDeleteUserExecutionException() throws Exception {
        // Given
        when(admin.alterUserScramCredentials(anyList())).thenReturn(alterUserScramCredentialsResult);
        when(alterUserScramCredentialsResult.all()).thenReturn(alterUserScramFuture);
        when(alterUserScramFuture.get()).thenThrow(new ExecutionException("User deletion failed", new RuntimeException()));
    ...
    @Test
    void testCheckAuthenticationSuccessProducer() throws Exception {
        // Given
        when(admin.describeCluster()).thenReturn(describeClusterResult);
        when(describeClusterResult.clusterId()).thenReturn(clusterIdFuture);
        when(clusterIdFuture.get()).thenReturn("test-cluster-id");
        ...
        when(admin.describeAcls(any(AclBindingFilter.class))).thenReturn(describeAclsResult);
        when(describeAclsResult.values()).thenReturn(describeAclsFuture);
        when(describeAclsFuture.get()).thenAnswer(invocation -> {
            AclBindingFilter filter = invocation.getArgument(0);
            if (filter.entryFilter().operation() == AclOperation.READ) {
                return Collections.emptyList();
            } else {
                return createMockAclBindings();
            }
        });
        ...
        assertNotNull(result);
        assertEquals(true, result.get("authenticated"));
        assertEquals("producer", result.get("role"));
    ...
    @Test
    void testCheckAuthenticationSuccessBoth() throws Exception {
        // Given
        when(admin.describeCluster()).thenReturn(describeClusterResult);
        when(describeClusterResult.clusterId()).thenReturn(clusterIdFuture);
        when(clusterIdFuture.get()).thenReturn("test-cluster-id");
    ...
    private Map<String, UserScramCredentialsDescription> createMockUserCredentials() {
        ...
        UserScramCredentialsDescription desc1 = new UserScramCredentialsDescription(List.of(info1));
        UserScramCredentialsDescription desc2 = new UserScramCredentialsDescription(List.of(info2));
    ...
    private Collection<AclBinding> createMockAclBindings() {
        ...
        return List.of(new AclBinding(topicPattern, readEntry));
    }
}
